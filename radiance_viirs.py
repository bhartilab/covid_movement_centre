...
-------------------------------------------------------------------------------
 ...
 Functions listed in alpabetical order by function name.
-------------------------------------------------------------------------------
"""
...
def preprocess_vnp46a1(hdf5_path, output_folder):
    ...
    try:
        ...
        # Mask radiance for 'probably clear' (cloud_detection_bitmask == 1)
        masked_for_probably_clear = ma.masked_where(
            cloud_detection_bitmask == 1, masked_for_fill_value, copy=True
        )

        # Mask radiance for 'probably cloudy' (cloud_detection_bitmask == 2)
        masked_for_probably_cloudy = ma.masked_where(
            cloud_detection_bitmask == 2, masked_for_probably_clear, copy=True
        )
